#!/usr/bin/env python3
"""
Test Stateful Conversational Pattern - InsuranceLiaisonAgent

Tests the stateful conversational pattern: maintains conversation history
across multiple interactions, providing context-aware responses.
...
6. Cost tracking
7. Stateful behavior verification
...
os.environ.setdefault("TEST_MAX_COST", "1.00") # Max cost for stateful test
...
logger = logging.getLogger("StatefulConversationalTest")
...
async def setup_platform_services():
    ...
    try:
        ...
        di_container = DIContainerService("test_stateful_conversational")
...
async def setup_agent(platform_services: Dict[str, Any]):
    """Initialize InsuranceLiaisonAgent (declarative)."""
    logger.info("🔧 Setting up InsuranceLiaisonAgent (declarative)...")
    
    try:
        # Import declarative version directly (same pattern as stateless test)
        ...
        # Initialize agent (match the actual constructor signature)
        ...
        logger.info("✅ InsuranceLiaisonAgent initialized")
        return agent
...
async def test_agent_initialization_and_config(agent):
    ...
    try:
        ...
        assert agent.agent_name == "InsuranceLiaisonAgent", "Agent name mismatch"
...
# ============================================================================
# TEST 2: Simple Conversational Request
# ============================================================================

async def test_simple_conversational_request(agent):
    """Test handling a simple conversational request."""
    test_name = "Simple Conversational Request"
    try:
        user_context = {"user_id": "test_user_1", "tenant_id": "test_tenant_1", "role": "manager"}
        
        request = {
            "message": "Hello, I need help with insurance migration.",
            "user_context": user_context
        }
        
        logger.info("   🧪 Making a simple conversational request...")
        
        result = await agent.handle_user_request(request)
        
        logger.info(f"   Response: {json.dumps(result, indent=2)}")
        
        assert result.get("type") == "liaison_response", "Response should be liaison_response type"
...
# ============================================================================
# TEST 3: Stateful Behavior (Conversation History)
# ============================================================================
...
async def test_stateful_behavior(agent):
    ...
    try:
        ...
        # First request
        request_1 = {
            "message": "What is insurance migration?",
            "user_context": user_context
        }
...
async def test_multi_turn_conversation(agent):
    ...
    try:
        ...
        request_1 = {
            "message": "I want to migrate my insurance policies.",
            "user_context": user_context
        }
        result_1 = await agent.handle_user_request(request_1)
        ...
        result_3 = await agent.handle_user_request(request_3)
        
        # Verify conversation history is growing
        history_1 = result_1.get("conversation_history_length", 0)
        history_2 = result_2.get("conversation_history_length", 0)
        history_3 = result_3.get("conversation_history_length", 0)
        
        assert history_3 >= history_2 >= history_1, "Conversation history should grow with each turn"
        assert history_3 <= agent.max_conversation_history, "History should not exceed max limit"
        
        log_test(test_name, "PASS", details={
            "Turn 1 History": history_1,
            "Turn 2 History": history_2,
            "Turn 3 History": history_3,
            "Max History Limit": agent.max_conversation_history
        })
        return True
    except AssertionError as e:
        log_test(test_name, "FAIL", str(e))
...
async def test_cost_tracking(agent):
    ...
    try:
        ...
        # Make a request
        request = {
            "message": "Help me understand insurance migration.",
            "user_context": user_context
        }
...
async def main():
    logger.info("================================================================================")
    logger.info("TEST: Stateful Conversational Pattern - InsuranceLiaisonAgent")
    logger.info("================================================================================")
    ...
    try:
        ...
        await test_simple_conversational_request(agent)
