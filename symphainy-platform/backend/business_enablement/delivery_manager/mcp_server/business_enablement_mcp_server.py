#!/usr/bin/env python3
"""
Business Enablement Realm MCP Server - Unified Pattern (Phase 3.2.5)

Exposes Business Enablement Service SOA APIs as MCP Tools for agent consumption.

UNIFIED PATTERN:
- Service defines SOA APIs via _define_soa_api_handlers()
...
class BusinessEnablementMCPServer(MCPServerBase):
    """
    Business Enablement Realm MCP Server - Unified Pattern (Phase 3.2.5)
    
    Exposes Business Enablement Service (DeliveryManagerService) SOA APIs as MCP Tools.
    Tools are automatically registered from service's _define_soa_api_handlers().
    ...
    def __init__(self, service, di_container):
        """
        Initialize Business Enablement Realm MCP Server.
        
        Args:
            service: DeliveryManagerService instance (or other Business Enablement service)
            di_container: DI Container for platform services
        """
        super().__init__(
            service_name="business_enablement_mcp",
    ...
    async def initialize(self):
        ...
        """
        try:
            self.logger.info("🔧 Initializing Business Enablement Realm MCP Server (unified pattern)...")
            
            # Get SOA API definitions from service
            if not hasattr(self.service, '_define_soa_api_handlers'):
                self.logger.warning(
                    f"⚠️ Service {self.service.__class__.__name__} does not define SOA APIs. "
                    f"No tools will be registered."
                )
                await super().initialize()
                return True
            
            soa_apis = self.service._define_soa_api_handlers()
            
            if not soa_apis:
                self.logger.warning("⚠️ No SOA APIs defined by service. No tools will be registered.")
                await super().initialize()
                return True
            ...
            for api_name, api_def in soa_apis.items():
                try:
                    ...
                    tool_name = f"business_enablement_{api_name}"
                    ...
                    self.register_tool(
                        ...
                        description=api_def.get("description", f"Business Enablement realm: {api_name}")
                    )
            ...
            self.logger.info(
                f"✅ Business Enablement Realm MCP Server initialized with {registered_count} tools "
            ...
            self.logger.error(f"❌ Failed to initialize Business Enablement Realm MCP Server: {e}")
            import traceback
    ...
    def get_usage_guide(self) -> Dict[str, Any]:
        """Return usage guide for Business Enablement Realm MCP Server."""
        return {
            "server_name": "business_enablement_mcp",
            "realm": "business_enablement",
            "description": "Business Enablement Realm MCP Server - exposes Business Enablement Service capabilities",
            "tools": list(self.soa_api_registry.values()),
            "usage": "Agents use MCP tools to interact with Business Enablement realm capabilities"
        }
