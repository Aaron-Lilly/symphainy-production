...
class ContentOrchestrator(OrchestratorBase):
    ...
    async def _get_file_parser_service(self):
        ...
        if self._file_parser_service is None:
            try:
                ...
                # Tier 2: Fallback - Import and initialize directly (Content realm service)
    ...
    async def process_file(
        ...
        try:
            ...
            self.logger.info(f"🔍 [process_file] Initial parse_options keys: {list(parse_options.keys()) if parse_options else 'None'}")
            self.logger.info(f"🔍 [process_file] copybook_file_id parameter: {copybook_file_id}")
            self.logger.info(f"🔍 [process_file] copybook_file_id in parse_options: {'copybook_file_id' in parse_options if parse_options else False}")
            
            # Handle copybook_file_id in parse_options - retrieve copybook data if needed
            if parse_options and "copybook_file_id" in parse_options:
                copybook_file_id_to_retrieve = parse_options.pop("copybook_file_id")
                self.logger.info(f"🔍 [process_file] Attempting to retrieve copybook file: {copybook_file_id_to_retrieve}")
                try:
                    # Retrieve copybook file
                    copybook_doc = await self._realm_service.retrieve_document(copybook_file_id_to_retrieve)
                    self.logger.info(f"🔍 [process_file] Copybook document retrieved: {copybook_doc is not None}")
                    if copybook_doc:
                        self.logger.info(f"🔍 [process_file] Copybook document keys: {list(copybook_doc.keys()) if isinstance(copybook_doc, dict) else 'Not a dict'}")
                        copybook_data = copybook_doc.get("file_content") or copybook_doc.get("data") or copybook_doc.get("content")
                        self.logger.info(f"🔍 [process_file] Copybook data type: {type(copybook_data)}, length: {len(copybook_data) if copybook_data else 0}")
                        if isinstance(copybook_data, str):
                            copybook_data = copybook_data.encode('utf-8')
                        elif not isinstance(copybook_data, bytes):
                            copybook_data = bytes(copybook_data) if copybook_data else b""
                        ...
                        # Add copybook data to parse_options
                        copybook_string = copybook_data.decode('utf-8') if copybook_data else ""
                        parse_options["copybook"] = copybook_string
                        self.logger.info(f"✅ Retrieved copybook data from file: {copybook_file_id_to_retrieve}, length: {len(copybook_string)}")
                    else:
                        self.logger.warning(f"⚠️ Copybook file not found: {copybook_file_id_to_retrieve}")
                except Exception as e:
                    self.logger.error(f"❌ Failed to retrieve copybook file {copybook_file_id_to_retrieve}: {e}")
                    import traceback
                    self.logger.error(f"Traceback: {traceback.format_exc()}")
            
            self.logger.info(f"🔍 [process_file] Final parse_options keys: {list(parse_options.keys()) if parse_options else 'None'}")
            self.logger.info(f"🔍 [process_file] Has 'copybook' in parse_options: {'copybook' in parse_options if parse_options else False}")
            ...
            # Extract preview data (first 20 records) for frontend display
            # This allows users to see actual parsed content without loading all data
            preview_records = []
            preview_tables = []
            
            # Debug: Log what we have to work with
            import sys
            sys.stderr.write(f"\n🔍🔍🔍 [process_file] Extracting preview data...\n")
            sys.stderr.write(f"🔍🔍🔍 [process_file] records type: {type(records)}, length: {len(records) if records else 0}\n")
            sys.stderr.write(f"🔍🔍🔍 [process_file] tables type: {type(tables)}, length: {len(tables) if tables else 0}\n")
            sys.stderr.flush()
            self.logger.error(f"🔍🔍🔍 [process_file] Extracting preview data - records: {len(records) if records else 0}, tables: {len(tables) if tables else 0}")
            
            if records and len(records) > 0:
                # Take first 20 records for preview
                preview_records = records[:20]
                sys.stderr.write(f"🔍🔍🔍 [process_file] Extracted {len(preview_records)} preview records\n")
                sys.stderr.flush()
                self.logger.error(f"🔍🔍🔍 [process_file] Extracted {len(preview_records)} preview records")
                # Sanitize preview records to ensure JSON-serializable
                preview_records = self._sanitize_for_json(preview_records)
                sys.stderr.write(f"🔍🔍🔍 [process_file] After sanitization: {len(preview_records)} preview records\n")
                sys.stderr.flush()
            else:
                sys.stderr.write(f"🔍🔍🔍 [process_file] No records found to extract preview from\n")
                sys.stderr.flush()
                self.logger.warning(f"⚠️ [process_file] No records found to extract preview from")
            
            if tables and len(tables) > 0:
                # For each table, include preview data (first 20 rows)
                for table in tables:
                    table_data = table.get("data", [])
                    sys.stderr.write(f"🔍🔍🔍 [process_file] Table has {len(table_data) if table_data else 0} rows\n")
                    sys.stderr.flush()
                    
                    # Debug: Check first row structure
                    if table_data and len(table_data) > 0:
                        first_row = table_data[0]
                        sys.stderr.write(f"🔍🔍🔍 [process_file] First table row type: {type(first_row)}\n")
                        if isinstance(first_row, dict):
                            sys.stderr.write(f"🔍🔍🔍 [process_file] First table row keys: {list(first_row.keys())[:5]}...\n")
                            sys.stderr.write(f"🔍🔍🔍 [process_file] First table row sample values: {dict(list(first_row.items())[:3])}\n")
                        elif isinstance(first_row, (list, tuple)):
                            sys.stderr.write(f"🔍🔍🔍 [process_file] First table row is list/tuple with {len(first_row)} items\n")
                            sys.stderr.write(f"🔍🔍🔍 [process_file] First table row sample: {list(first_row)[:3]}\n")
                        sys.stderr.flush()
                    
                    # Convert records (dicts) to rows (arrays) for preview_grid
                    # Frontend expects preview_grid to be string[][] (rows of cells), not Dict[]
                    preview_rows = []
                    if table_data:
                        columns = table.get("columns", [])
                        # If columns not provided, extract from first record
                        if not columns and len(table_data) > 0 and isinstance(table_data[0], dict):
                            columns = list(table_data[0].keys())
                        
                        # Convert each record (dict) to a row (array of values)
                        for record in table_data[:20]:
                            if isinstance(record, dict):
                                # Extract values in column order
                                row = [str(record.get(col, '')) for col in columns]
                                preview_rows.append(row)
                            elif isinstance(record, (list, tuple)):
                                # Already a row, just convert to list and sanitize
                                preview_rows.append([str(val) for val in record])
                    
                    table_preview = {
                        "columns": table.get("columns", []),
                        "row_count": table.get("row_count", 0),
                        "preview_data": self._sanitize_for_json(preview_rows) if preview_rows else []
                    }
                    preview_tables.append(table_preview)
                    sys.stderr.write(f"🔍🔍🔍 [process_file] Added table preview with {len(table_preview.get('preview_data', []))} preview rows\n")
                    
                    # Debug: Check sanitized preview data
                    if table_preview.get("preview_data") and len(table_preview["preview_data"]) > 0:
                        sanitized_first_row = table_preview["preview_data"][0]
                        sys.stderr.write(f"🔍🔍🔍 [process_file] Sanitized first preview row type: {type(sanitized_first_row)}\n")
                        if isinstance(sanitized_first_row, dict):
                            sys.stderr.write(f"🔍🔍🔍 [process_file] Sanitized first preview row keys: {list(sanitized_first_row.keys())[:5]}...\n")
                            # Show first 3 key-value pairs
                            sample_items = dict(list(sanitized_first_row.items())[:3])
                            sys.stderr.write(f"🔍🔍🔍 [process_file] Sanitized first preview row sample: {sample_items}\n")
                            # Show actual values (not just keys)
                            for key, val in list(sanitized_first_row.items())[:3]:
                                sys.stderr.write(f"🔍🔍🔍 [process_file]   {key}: {repr(val)} (type: {type(val).__name__})\n")
                        sys.stderr.flush()
            else:
                sys.stderr.write(f"🔍🔍🔍 [process_file] No tables found to extract preview from\n")
                sys.stderr.flush()
                self.logger.warning(f"⚠️ [process_file] No tables found to extract preview from")
            
            # Build parse_summary with metadata + preview data
            parse_summary = {
                ...
                "format": sanitized_parse_result.get("parsing_type", "json_structured"),  # For frontend compatibility
                ...
                # Include preview data for frontend display
                "preview_records": preview_records,  # First 20 records
                "preview_tables": preview_tables,  # First 20 rows of each table
                "text_content": sanitized_parse_result.get("text_content", ""),  # Text representation if available
                # For backward compatibility, also include as "parsed_data" structure
                "parsed_data": {
                    "format": sanitized_parse_result.get("parsing_type", "json_structured"),
                    "chunks": preview_records,  # Preview records as chunks
                    "structured_data": {
                        "tables": preview_tables,
                        "records": preview_records
                    },
                    "preview_grid": preview_tables[0].get("preview_data", []) if preview_tables else [],
                    "text": sanitized_parse_result.get("text_content", "")
                }
            }
            
            # Explicitly remove any full data that might have leaked through (keep only preview)
            # This ensures we never return full tables/records arrays (only preview)
            ...
            import sys
            sys.stderr.write(f"\n🔍🔍🔍 [process_file] Returning result with parse_summary keys: {list(parse_summary.keys())}\n")
            sys.stderr.write(f"🔍🔍🔍 [process_file] parse_summary.preview_records count: {len(parse_summary.get('preview_records', []))}\n")
            sys.stderr.write(f"🔍🔍🔍 [process_file] parse_summary.preview_tables count: {len(parse_summary.get('preview_tables', []))}\n")
            if parse_summary.get('preview_records'):
                sys.stderr.write(f"🔍🔍🔍 [process_file] First preview record keys: {list(parse_summary['preview_records'][0].keys()) if parse_summary['preview_records'] else 'None'}\n")
            sys.stderr.flush()
            self.logger.error(f"🔍🔍🔍 [process_file] Returning result with parse_summary keys: {list(parse_summary.keys())}")
            self.logger.error(f"🔍🔍🔍 [process_file] parse_summary.preview_records count: {len(parse_summary.get('preview_records', []))}")
            self.logger.error(f"🔍🔍🔍 [process_file] parse_summary.preview_tables count: {len(parse_summary.get('preview_tables', []))}")
    ...
    async def embed_content(
        ...
        try:
            ...
            # Get EmbeddingService (enabling service)
            embedding_service = await self.get_enabling_service("EmbeddingService")
            if not embedding_service:
                self.logger.error("❌ EmbeddingService not available")
                return {
                    "success": False,
    ...
    async def get_pillar_summary(
        ...
        """
        Get Content Pillar summary for Business Outcomes page (NEW semantic API method).
        
        Returns the 3-way summary (textual, tabular, visualizations) of processed files
        and semantic data model for display in Business Outcomes pillar.
        
        Args:
            session_id: Optional session identifier (for filtering)
            user_id: Optional user identifier (defaults to context user)
        ...
        """
        try:
            self.logger.info(f"📊 Getting Content pillar summary for session: {session_id}")
            
            # Get user_id from context if not provided
            if not user_id:
                user_context = self._realm_service.get_user_context() if hasattr(self._realm_service, 'get_user_context') else None
                user_id = user_context.get("user_id") if user_context else "anonymous"
            
            # List uploaded files for user
            files_result = await self.list_uploaded_files(user_id)
            if not files_result.get("success"):
                return {
                    "success": False,
                    "error": "Failed to list files",
                    "error_details": files_result.get("error", "Unknown error"),
            ...
            # Categorize files
            structured_files = [f for f in files if f.get("content_type") == "structured"]
            unstructured_files = [f for f in files if f.get("content_type") == "unstructured"]
            hybrid_files = [f for f in files if f.get("content_type") == "hybrid"]
            parsed_files = [f for f in files if f.get("parsed", False)]
            
            # Generate textual summary
            total_files = len(files)
            structured_count = len(structured_files)
            unstructured_count = len(unstructured_files)
            hybrid_count = len(hybrid_files)
            parsed_count = len(parsed_files)
            
            textual = (
                f"Processed {total_files} file(s): {structured_count} structured, "
                f"{unstructured_count} unstructured, {hybrid_count} hybrid. "
                f"{parsed_count} file(s) have been parsed. "
                f"Semantic data model creation is in progress (embeddings and semantic graphs will be available soon)."
            )
            
            # Generate tabular summary
            tabular_rows = []
            for file in files[:10]:  # Limit to first 10 for summary
                tabular_rows.append([
                    file.get("ui_name", file.get("original_filename", "Unknown")),
                    file.get("content_type", "unknown").title(),
                    "Processed" if file.get("parsed", False) else "Pending",
                    f"{file.get('size_bytes', 0) / 1024:.1f} KB" if file.get("size_bytes") else "N/A"
                ])
            
            tabular = {
                "columns": ["File Name", "Type", "Status", "Size"],
                "rows": tabular_rows,
                "summary_stats": {
                    "total_rows": len(files),
                    "key_metrics": {
                        "total_files": total_files,
                        "structured_files": structured_count,
                        "unstructured_files": unstructured_count,
            ...
            visualizations = [
                {
                    "visualization_id": "content_file_types",
                    "chart_type": "pie",
                    "library": "recharts",
                    "title": "File Types Distribution",
                    "rationale": "Shows breakdown of structured vs unstructured files",
                    "chart_data": [
                        {"name": "Structured", "value": structured_count},
                        {"name": "Unstructured", "value": unstructured_count},
                        {"name": "Hybrid", "value": hybrid_count}
                    ],
                    "colors": ["#8884d8", "#82ca9d", "#ffc658"]
                },
                {
                    "visualization_id": "content_processing_status",
                    "chart_type": "bar",
                    "library": "recharts",
                    "title": "File Processing Status",
                    "rationale": "Shows how many files have been parsed",
                    "chart_data": [
                        {"name": "Parsed", "value": parsed_count},
                        {"name": "Pending", "value": total_files - parsed_count}
                    ],
                    "colors": ["#8884d8", "#ffc658"]
                }
            ]
            
            # Semantic data model summary (placeholder for future implementation)
            semantic_data_model = {
                "structured_files": {
                    "count": structured_count,
                    "total_columns": 0,  # TODO: Get from actual semantic data
                    "total_embeddings": 0,  # TODO: Get from Arango
                    "semantic_ids_matched": 0  # TODO: Get from Arango
                },
                "unstructured_files": {
                    "count": unstructured_count,
                    "total_entities": 0,  # TODO: Get from Arango
                    "total_relationships": 0,  # TODO: Get from Arango
                    "semantic_ids_matched": 0  # TODO: Get from Arango
                },
                "arango_storage": {
                    "collections": ["structured_embeddings", "semantic_graph_nodes", "semantic_graph_edges"],
                    "total_documents": 0,  # TODO: Get from Arango
                    "status": "planned"  # Indicates semantic data model is planned but not yet implemented
                }
            }
            
            self.logger.info(f"✅ Content pillar summary generated: {total_files} files")
            ...
            return {
                "success": False,
                "error": "Failed to get Content pillar summary",
                "error_details": str(e),
                "timestamp": datetime.utcnow().isoformat()
            }
    ...
    async def embed_content(
        ...
        try:
            # Get EmbeddingService using four-tier pattern (same as FileParserService)
            embedding_service = None
            try:
                embedding_service = await self.get_enabling_service("EmbeddingService")
            except Exception as discovery_error:
                self.logger.debug(f"Curator discovery failed for EmbeddingService: {discovery_error}")
                embedding_service = None
            
            if not embedding_service:
                # Tier 2: Fallback - Import and initialize directly
                self.logger.warning("⚠️ EmbeddingService not found via Curator, initializing directly")
                try:
                    ...
                    self.logger.info("✅ EmbeddingService initialized directly")
