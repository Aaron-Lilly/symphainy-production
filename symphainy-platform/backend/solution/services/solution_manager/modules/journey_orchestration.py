...
class JourneyOrchestration:
    ...
    async def orchestrate_journey(
        ...
        try:
            ...
            # Call Journey Manager's design_journey method (with user_context)
            if hasattr(journey_manager, "design_journey"):
                # Extract journey_request from journey_context if needed
                journey_request = journey_context.get("journey_request", journey_context)
                journey_result = await journey_manager.design_journey(journey_request, user_context=user_context)
