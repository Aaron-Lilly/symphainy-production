#!/usr/bin/env python3
"""
Business Outcomes Realm MCP Server - Unified Pattern (Phase 3.2.5)

Exposes Business Outcomes Journey Orchestrator SOA APIs as MCP Tools for agent consumption.
...
class BusinessOutcomesMCPServer(MCPServerBase):
    ...
    Exposes Business Outcomes Journey Orchestrator SOA APIs as MCP Tools.
    ...
    def __init__(self, orchestrator, di_container):
        """
        Initialize Business Outcomes Realm MCP Server.
        
        Args:
            orchestrator: BusinessOutcomesJourneyOrchestrator instance
    ...
    async def initialize(self):
        ...
        """
        try:
            self.logger.info("🔧 Initializing Business Outcomes Realm MCP Server (unified pattern)...")
            ...
            for api_name, api_def in soa_apis.items():
                try:
                    ...
                    self.register_tool(
                        ...
                        description=api_def.get("description", f"Business Outcomes realm: {api_name}")
                    )
            ...
            self.logger.info(
                f"✅ Business Outcomes Realm MCP Server initialized with {registered_count} tools "
            ...
            self.logger.error(f"❌ Failed to initialize Business Outcomes Realm MCP Server: {e}")
            import traceback
    ...
    def get_usage_guide(self) -> Dict[str, Any]:
        """Return usage guide for Business Outcomes Realm MCP Server."""
        return {
            "server_name": "business_outcomes_mcp",
            "realm": "journey",
            "description": "Business Outcomes Realm MCP Server - exposes Business Outcomes Journey Orchestrator capabilities",
            "tools": list(self.soa_api_registry.values()),
            "usage": "Agents use MCP tools to interact with Business Outcomes realm capabilities"
        }
