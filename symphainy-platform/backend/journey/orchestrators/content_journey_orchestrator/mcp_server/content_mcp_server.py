#!/usr/bin/env python3
"""
Content Realm MCP Server - Unified Pattern (Phase 3.2.5)

Exposes Content Journey Orchestrator SOA APIs as MCP Tools for agent consumption.
...
class ContentMCPServer(MCPServerBase):
    ...
    Exposes Content Journey Orchestrator SOA APIs as MCP Tools.
    ...
    def __init__(self, orchestrator, di_container):
        ...
        Args:
            orchestrator: ContentJourneyOrchestrator instance
    ...
    async def initialize(self):
        ...
        """
        try:
            self.logger.info("🔧 Initializing Content Realm MCP Server (unified pattern)...")
            ...
            self.logger.info(
                f"✅ Content Realm MCP Server initialized with {registered_count} tools "
    ...
    def get_usage_guide(self) -> Dict[str, Any]:
        """Return usage guide for Content Realm MCP Server."""
        return {
            "server_name": "content_mcp",
            "realm": "content",
            "description": "Content Realm MCP Server - exposes Content Journey Orchestrator capabilities",
            "tools": list(self.soa_api_registry.values()),
            "usage": "Agents use MCP tools to interact with Content realm capabilities"
        }
