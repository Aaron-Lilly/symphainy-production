...
# Import UserContext for agent method calls
try:
...
class UnstructuredAnalysisWorkflow:
    ...
    async def execute(
        ...
        try:
            ...
            # ✅ Extract workflow_id from user_context for end-to-end correlation
            workflow_id = None
            if user_context:
                workflow_id = user_context.get("workflow_id") if isinstance(user_context, dict) else getattr(user_context, "workflow_id", None)
    ...
    async def _generate_insights(
        ...
        """Generate insights summary using InsightsGeneratorService with solution context."""
        try:
            # Extract solution context from options
            solution_context = options.get("solution_context")
            user_goals = options.get("user_goals", "")
            strategic_focus = options.get("strategic_focus", "")
            
            # Build context-aware prompt if solution context available
            context_prompt = ""
            if solution_context or user_goals or strategic_focus:
                context_prompt = f"""
...
"""
    ...
    async def _perform_aar_analysis(
        ...
        """
        Perform Navy AAR-specific analysis using agentic-forward pattern.
        
        ✅ AGENTIC-FORWARD PATTERN:
        1. Agent critical reasoning first (InsightsSpecialistAgent)
        2. Service execution based on agent's strategic decisions
        """
        try:
            # ✅ STEP 1: Get InsightsSpecialistAgent for critical reasoning
            specialist_agent = await self.orchestrator._get_insights_specialist_agent()
            if not specialist_agent:
                self.logger.warning("⚠️ Insights Specialist Agent not available, falling back to service-only AAR analysis")
                # Fallback to service-only approach (for backward compatibility)
                return await self._perform_aar_analysis_service_only(text_data, processing_result, options)
            
            # ✅ STEP 2: Agent critical reasoning - analyze AAR document structure
            # Convert user_context to UserContext if needed
            user_context = options.get("user_context")
            if user_context and not isinstance(user_context, dict):
                # Already a UserContext object
                pass
            elif user_context and isinstance(user_context, dict):
                # Convert dict to UserContext-like object
                from utilities import UserContext
            ...
            # Agent performs critical reasoning and extraction
            agent_result = await specialist_agent.analyze_aar_document(
                text_data=text_data,
                processing_result=processing_result,
            ...
            if not agent_result.get("success"):
                self.logger.warning(f"⚠️ Agent AAR analysis failed: {agent_result.get('error')}, falling back to service-only")
                return await self._perform_aar_analysis_service_only(text_data, processing_result, options)
            
            # Extract AAR data from agent result
            aar_data = agent_result.get("aar_data", {})
            reasoning = agent_result.get("reasoning", "")
            extraction_strategy = agent_result.get("extraction_strategy", {})
            
            self.logger.info(f"✅ AAR analysis complete via agentic-forward pattern: {len(aar_data.get('lessons_learned', []))} lessons, {len(aar_data.get('risks', []))} risks")
            
            return {
                "success": True,
                "aar_data": {
                    "lessons_learned": aar_data.get("lessons_learned", []),
                    "risks": aar_data.get("risks", []),
                    "recommendations": aar_data.get("recommendations", []),
                    "timeline": aar_data.get("timeline", [])
                },
            ...
            self.logger.warning(f"⚠️ AAR analysis failed: {e}, falling back to service-only")
            # Fallback to service-only approach
            return await self._perform_aar_analysis_service_only(text_data, processing_result, options)
    
    async def _perform_aar_analysis_service_only(
        self,
        text_data: str,
        processing_result: Dict[str, Any],
        options: Dict[str, Any]
        ...
        """
        Fallback AAR analysis using APGProcessorService (service-only, no agent).
        
        This is kept for backward compatibility when agent is not available.
        """
        try:
            # Access APGProcessorService from orchestrator
            apg_processor = await self.orchestrator._get_apg_processor_service()
            ...
            self.logger.warning(f"⚠️ Service-only AAR analysis failed: {e}, using empty AAR data")
            return {
                "success": True,
                "aar_data": {
                    "lessons_learned": [],
