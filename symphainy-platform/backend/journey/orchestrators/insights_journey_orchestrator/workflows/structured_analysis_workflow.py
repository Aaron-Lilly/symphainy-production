...
class StructuredAnalysisWorkflow:
    ...
    async def execute(
        ...
        try:
            ...
            # ✅ If content_id is provided in options, use it (for EDA analysis which requires embeddings)
            if options and options.get("content_id"):
                data_content["content_id"] = options.get("content_id")
    ...
    async def _analyze_data(
        ...
        try:
            ...
            # DataAnalyzerService.run_eda_analysis() requires a content_id with schema embeddings
            # We need to get the parsed file and its associated content_metadata_id
            file_id = data_content.get("file_id")
            content_id = data_content.get("content_id") or data_content.get("metadata_id")
                ...
                try:
                    ...
                    if content_steward:
                        ...
                        if parsed_file:
                            ...
                            content_id = (
                                parsed_metadata.get("content_metadata_id") or
                                parsed_metadata.get("content_id") or
                                parsed_file.get("content_metadata_id") or
                                parsed_file.get("content_id")
                            )
                            
                            # If still no content_id, check if we can use parsed_data directly
                            # For now, if no embeddings exist, return a helpful error
                            if not content_id:
                                self.logger.warning(f"⚠️ No content_metadata_id found for file_id {file_id}. Embeddings may not have been created yet.")
                                return {
                                    "success": False,
                                    "error": "No content_metadata_id found. Please create embeddings for this file first using the Content Pillar.",
                                    "suggestion": "Use /api/v1/content-pillar/create-embeddings to create embeddings before analysis"
                                }
                except Exception as e:
                    self.logger.warning(f"⚠️ Failed to get parsed file for content_id lookup: {e}")
            
            if not content_id:
                return {
                    "success": False,
                    "error": "content_id is required for EDA analysis. Please ensure the file has been parsed and embeddings have been created.",
                    "suggestion": "Use /api/v1/content-pillar/create-embeddings to create embeddings before analysis"
                }
            
            # Use run_eda_analysis() method (correct method name for DataAnalyzerService)
            analysis_types = options.get("analysis_types", ["statistics", "correlations", "distributions", "missing_values"])
            
            analysis_result = await data_analyzer.run_eda_analysis(
