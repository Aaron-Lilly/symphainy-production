#!/bin/bash
# Helper script to start infrastructure and run Foundation integration tests.
#
# WHAT: Start infrastructure and run foundation integration tests with utility compliance
...
echo -e "${BLUE}🚀 Foundation Integration Test Runner${NC}"
echo "=========================================="
echo ""
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --file)
            ...
            ;;
        --utility-compliance)
            TEST_MARKER="utility_compliance"
            shift
            ;;
        --foundation-integration)
            TEST_MARKER="foundation_integration"
            shift
            ;;
        --help)
            ...
            echo "  --marker MARKER       Run only tests with specific marker"
            echo "  --file FILE           Run specific test file"
            echo "  --utility-compliance  Run only utility compliance tests"
            echo "  --foundation-integration  Run only foundation integration tests"
...
if [ "$RUN_TESTS" = true ]; then
    echo -e "${GREEN}🧪 Running foundation integration tests...${NC}"
    echo ""
    
    # Build test command
    if [ -n "$TEST_FILE" ]; then
        TEST_CMD="python3 -m pytest tests/integration/foundations/$TEST_FILE -v --tb=short"
    else
        TEST_CMD="python3 -m pytest tests/integration/foundations/ -v --tb=short"
    fi
    ...
    if eval $TEST_CMD; then
        echo ""
        echo -e "${GREEN}✅ All foundation integration tests passed!${NC}"
        TEST_RESULT=0
    else
        echo ""
        echo -e "${RED}❌ Some foundation integration tests failed${NC}"
        TEST_RESULT=1
    fi
else
