#!/usr/bin/env python3
"""
Security Guard Service - Corrected Infrastructure Mapping

Updated clean rebuild with correct infrastructure mapping:
- Authentication via Supabase + JWT ✅
- Authorization via Supabase ✅
- Session management via Redis ✅
- Tenant management via Supabase (not Redis) ✅
- Policy management via Supabase ✅

WHAT (Smart City Role): I provide platform security with correct infrastructure integration
HOW (Service Implementation): I use SmartCityRoleBase with corrected infrastructure abstractions
"""
...
class SecurityGuardService(SmartCityRoleBase, SecurityGuardServiceProtocol):
    ...
    with corrected infrastructure abstractions for proper database mapping.
    ...
    def __init__(self, di_container: Any):
        """Initialize Security Guard Service with corrected infrastructure integration."""
        super().__init__(
        ...
        self.tenant_abstraction = None  # Now uses Supabase
        ...
        if self.logger:
            self.logger.info("✅ Security Guard Service (Corrected Infrastructure Mapping) initialized")
    
    async def initialize(self) -> bool:
        """Initialize Security Guard Service with corrected infrastructure connections."""
        try:
            if self.logger:
                self.logger.info("🚀 Initializing Security Guard Service with corrected infrastructure connections...")
            ...
            if self.logger:
                self.logger.info("✅ Security Guard Service (Corrected Infrastructure) initialized successfully")
    ...
    async def _initialize_infrastructure_connections(self):
        """Initialize connections to corrected infrastructure abstractions."""
        try:
            if self.logger:
                self.logger.info("🔌 Connecting to corrected security infrastructure abstractions...")
            ...
            # Get Tenant Abstraction (Supabase - CORRECTED)
            ...
            if self.logger:
                self.logger.info("✅ Corrected security infrastructure connections established:")
                ...
                self.logger.info("  - Tenant Management (Supabase): ✅ CORRECTED")
                self.logger.info("  - Policy Management (Supabase): ✅")
            
        except Exception as e:
            if self.logger:
                self.logger.error(f"❌ Failed to connect to corrected security infrastructure: {str(e)}")
            raise e
    ...
    async def _initialize_soa_api_exposure(self):
        ...
        self.soa_apis = {
            ...
            "get_tenant_context": {
                ...
                "method": "GET",
                "description": "Get tenant security context from Supabase",
    ...
    async def _initialize_mcp_tool_integration(self):
        ...
        self.mcp_tools = {
            ...
            "tenant_security": {
                ...
                "description": "Manage tenant security contexts from Supabase",
                "parameters": ["tenant_id", "security_config", "action"]
            }
        }
    
    async def _register_security_guard_capabilities(self) -> Dict[str, Any]:
        """Register Security Guard Service capabilities with corrected infrastructure mapping."""
        return {
            "service_name": "SecurityGuardService",
            ...
            "infrastructure_connections": {
                ...
                "tenant_management": "Supabase",  # CORRECTED
    ...
    # ============================================================================
    # TENANT MANAGEMENT METHODS WITH CORRECTED INFRASTRUCTURE INTEGRATION
    # ============================================================================
    
    async def get_tenant_context(self, tenant_id: str) -> Dict[str, Any]:
        """Get tenant context using Supabase infrastructure (CORRECTED)."""
        try:
            if not self.is_infrastructure_connected:
                raise Exception("Infrastructure not connected")
            
            # Use Tenant Abstraction (Supabase - CORRECTED)
            tenant_context = await self.tenant_abstraction.get_tenant_context(tenant_id)
            
            if tenant_context:
                if self.logger:
                    self.logger.info(f"✅ Tenant context retrieved from Supabase: {tenant_id}")
                ...
                return {
                    "tenant_id": tenant_id,
                    "error": "Tenant not found in Supabase",
                    "status": "error"
                }
                
        except Exception as e:
            if self.logger:
                self.logger.error(f"❌ Error getting tenant context from Supabase: {str(e)}")
            return {
                "tenant_id": tenant_id,
                "error": str(e),
                "status": "error"
    ...
    async def validate_tenant_access(self, user_id: str, tenant_id: str) -> bool:
        """Validate tenant access using Supabase infrastructure (CORRECTED)."""
        try:
            if not self.is_infrastructure_connected:
                raise Exception("Infrastructure not connected")
            ...
            if self.logger:
                self.logger.info(f"✅ Tenant access validation via Supabase: {user_id} -> {tenant_id}: {'✅' if has_access else '❌'}")
            
            return has_access
            
        except Exception as e:
            if self.logger:
                self.logger.error(f"❌ Error validating tenant access via Supabase: {str(e)}")
            return False
    ...
    async def validate_infrastructure_mapping(self) -> Dict[str, Any]:
        """Validate that corrected infrastructure mapping is working correctly."""
        try:
            validation_results = {
                "authentication_supabase_jwt": False,
                "authorization_supabase": False,
                "session_management_redis": False,
                "tenant_management_supabase": False,  # CORRECTED
            ...
            # Test Tenant Management (Supabase - CORRECTED)
            try:
                if self.tenant_abstraction:
                    test_result = await self.tenant_abstraction.health_check()
                    validation_results["tenant_management_supabase"] = True
            except Exception as e:
                if self.logger:
                    self.logger.warning(f"⚠️ Tenant Management (Supabase) test failed: {str(e)}")
            ...
            validation_results["overall_status"] = all([
                ...
                validation_results["tenant_management_supabase"],  # CORRECTED
                validation_results["policy_management_supabase"]
            ])
            
            if self.logger:
                self.logger.info("🔍 Corrected security infrastructure mapping validation completed:")
                ...
                self.logger.info(f"  - Tenant Management (Supabase): {'✅' if validation_results['tenant_management_supabase'] else '❌'} CORRECTED")
            ...
            if self.logger:
                self.logger.error(f"❌ Error validating corrected security infrastructure mapping: {str(e)}")
            return {
                "authentication_supabase_jwt": False,
                "authorization_supabase": False,
                "session_management_redis": False,
                "tenant_management_supabase": False,
                "policy_management_supabase": False,
                "overall_status": False,
                "error": str(e)
            }
    ...
    async def get_service_capabilities(self) -> Dict[str, Any]:
        """Get service capabilities with corrected infrastructure status."""
        try:
            return {
                "service_name": "SecurityGuardService",
                "service_type": "security_enforcement",
